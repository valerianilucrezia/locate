...
setwd('/orfeo/scratch/area/lvaleriani/locate/scripts/organoid/')
source('/orfeo/scratch/area/lvaleriani/COLO829/nf_analysis/utils.R')
out <- '/orfeo/cephfs/scratch/area/lvaleriani/locate/scripts/organoid/out/data'

option_list = list(
  make_option(c("-s", "--sample"), type="character", default="PDO61",
...
all_data <- lapply(1:22, FUN = function(chr){
  print(chr)
  path_phase <- paste0('/orfeo/scratch/area/lvaleriani/nf-locate/results_organoids/haplotagphase/',sample,'/',sample,'_chr',chr,'_phased.vcf.gz')
  path_shapeit <- paste0('/orfeo/scratch/area/lvaleriani/nf-locate/results_organoids/shapeit4/',sample,'/shapeit_',sample,'-chr',chr,'.vcf')
  path_normal <- paste0('/orfeo/scratch/area/lvaleriani/nf-locate/results_organoids/whatshap/', sample, '/', sample, '_chr', chr,'.vcf')
  ...
  phase <- read_vcf(path_phase) %>%
    filter(REF %in% c('A', 'G', 'C', 'T'),
           ALT %in% c('A', 'G', 'C', 'T')) %>%
    mutate(BAF_H1_P = ifelse(H1 == 1, BAF, 1-BAF)) %>%
    mutate(BAF_H2_P = ifelse(H2 == 1, BAF, 1-BAF))
  ...
  join <- phase %>%
    left_join(shapeit, by = join_by(CHROM, POS), suffix = c('_P', '_SI')) %>%
    filter(FILTER_P == 'PASS') %>%
  ...
  join <- join %>% filter(gt_DP_T>15) %>% filter(QUAL_P>15)
