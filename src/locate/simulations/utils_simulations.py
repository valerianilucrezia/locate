...
def plot_simulations(snp, snv, purity, coverage, ploidy, path='', save = False):
    """_summary_
    ...
    axes[0].set_title(f'Purity = {purity}, Coverage = {coverage}, Ploidy = {ploidy}')
    fig.tight_layout()
...
def plot_results_simulations(snp, snv, res, params, purity, coverage, ploidy):
    """_summary_
    ...
    axes[0].set_title(f'True Purity = {purity}, True Ploidy = {ploidy}, Coverage = {coverage}')
    try:
    ...
    try:
        inf_ploidy = float(params['ploidy'])
    except:
        inf_ploidy = 0
        
    
    axes[3].set_title(f'Inferred Purity = {round(inf_purity, 2)}, Inferred Ploidy = {round(inf_ploidy, 2)}')
    fig.tight_layout()
